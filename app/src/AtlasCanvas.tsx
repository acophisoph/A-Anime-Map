...
export function AtlasCanvas({ points, mode, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        await app.init({ background: '#05080f', resizeTo: hostRef.current!, antialias: true });
        ...
        setCanvasError(String(error?.message ?? error));
      }
    };
  ...
  if (canvasError) {
    return <div className="atlas-canvas" style={{ padding: '1rem' }}>Canvas failed to initialize: {canvasError}</div>;
  }
